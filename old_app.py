...
# ── Model loading ────────────────────────────────────────────────────
model_loaded = os.path.exists("social_virality_xgb_model.pkl")
if model_loaded:
    model = joblib.load("social_virality_xgb_model.pkl")
    tfidf = joblib.load("virality_tfidf_vectorizer.pkl")
else:
    model = tfidf = None
...
# ════════════════════════════════════════════════════════════════════
# SECTION 1 — POST VIRALITY
# ════════════════════════════════════════════════════════════════════
st.markdown("""
<div class="vl-section-head" style="margin-bottom:0.5rem;">
...
with pcol_out:
    if p_title.strip():
        ...
        length_sc = min(title_len / 80, 1.0)
        ...
        <div class="vl-result-box">
          ...
          <div class="vl-bars">
            ...
            {bar("Comment length signal", length_sc, "#a89cff")}
          ...
          <div class="vl-tip">{'Peak hour — strong timing window' if is_peak else 'Off-peak — consider posting at 12–15h or 20–22h UTC'}</div>
        </div>
        """, unsafe_allow_html=True)
    else:
...
# ── Divider ──────────────────────────────────────────────────────────
st.markdown('<div class="vl-divider" style="margin-top:2.2rem;"></div>', unsafe_allow_html=True)
 
 
# ════════════════════════════════════════════════════════════════════
# SECTION 2 — COMMENT VIRALITY
# ════════════════════════════════════════════════════════════════════
st.markdown("""
<div class="vl-section-head" style="margin-bottom:0.5rem;">
...
with ccol_in:
    ...
    c_post_title = st.text_input("Parent post title (optional, improves accuracy)", placeholder="e.g. Best way to learn ML?", key="c_ptitle")
...
with ccol_out:
    if c_body.strip():
        ...
        if model_loaded:
            title_text = c_post_title if c_post_title.strip() else "untitled"
            tfidf_vec  = tfidf.transform([title_text])
            tfidf_cols = {f"title_word_{w}": float(tfidf_vec[0, i])
            ...
            pred_c   = max(1, int(np.expm1(log_pred)))
        else:
            # Pure 0-1 weighted average, no logs, no overflow
            virality = (
                parent_sc * 0.35 +
                depth_sc  * 0.25 +
                timing_sc * 0.20 +
